...
//Connection Settings @0-39A4E907
...
//Initialize Common Variables @0-7CF37B1A
...
//CCLoginUser @0-1C999AC0
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT id_usuarios, nivel, clave FROM usuarios_cosemar WHERE user=" . $db->ToSQL($login, ccsText) . " AND clave=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("id_usuarios"));
